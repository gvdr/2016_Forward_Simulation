    ...
    for(i in ii:length(extlin)){
        if(inherits == "copy"){
            if(i != length(extlin)){
                ...
                newint <- evo_qrate_integrate(edgelist, extlin[[i]], extlin[[i]]$time, extlin[[i-1]]$time, q01, current_interactions)
                tmp <- rbind(tmp, newint)
                edgelist[seq(sum(edgelist[,1] != 0) + 1, sum(edgelist[,1] != 0) + dim(tmp)[1]), ] <- tmp
            }
        } else {
            tmp <- int_exact_copy(
                current_interactions = edgelist[which(edgelist$time == brtimes[i]), ],
                mom_number = extlin[[i]]$mom,
                child_l_number = extlin[[i]]$childs[1],
